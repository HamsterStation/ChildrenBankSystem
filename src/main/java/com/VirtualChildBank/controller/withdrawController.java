...
public class withdrawController {
    ...
    void onSelectAction(MouseEvent event) {
        ...
        if (selectedAccount != null) {
            // 根据账户类型决定加载哪个FXML文件
            String fxmlPath = selectedAccount.getAccountType().equals("saving") ? "/fxml/savingWithdraw.fxml" : "/fxml/currentWithdraw.fxml";
            try {
                // 调用重载的changeView方法并获取控制器
                Object controller = changeView(fxmlPath, true);

                // 假设控制器有一个方法setAccount来设置账户信息
                if (controller instanceof savingWithdrawController && "saving".equals(selectedAccount.getAccountType())) {
                    ((savingWithdrawController) controller).setAccount(selectedAccount);
                } else if (controller instanceof currentWithdrawController && "current".equals(selectedAccount.getAccountType())) {
                    ((currentWithdrawController) controller).setAccount(selectedAccount);
                }
